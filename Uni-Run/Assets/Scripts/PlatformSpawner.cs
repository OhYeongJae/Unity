...
public class PlatformSpawner : MonoBehaviour {
    ...
    void Update() {
        ...
        // 마지막 배치 시점에서 timeBestSpawn 이상 시간이 플렀다면
        if(Time.time >= lastSpawnTime + timeBetSpawn)
        {
            // 기록된 마지막 배치 시점을 현재 시점으로 갱신
            lastSpawnTime = Time.time;

            // 다음배치까지의 시간 간격을 timeBetSpawnMin, timeBetSpawnMax사이에서 랜덤 설정
            timeBetSpawn = Random.Range(timeBetSpawnMin, timeBetSpawnMax);

            // 배치할 위치의 높이를 yMin과 yMax 사이에서 랜덤 설정
            float yPos = Random.Range(yMin, yMax);

            // 사용할 현재 순번의 발판 게임 오브젝트를 비활성화하고 즉기 다시 활성화
            // 이때 발판의 Platform컴포넌트의 OnEnable메서드가 실행됨
